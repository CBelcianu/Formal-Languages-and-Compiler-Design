...
from Scanner import tokenResolver, isIdentifier, isConstant
...
with open(fileName, 'r') as file:
    ...
    for line in file:
        lineNo += 1
        for token in tokenResolver(line[0:-1], separators):
            if token in toEncode[2:-1]:
                pif.add(codification[token], -1)
            elif isIdentifier(token):
                uid = symbolTable.add(len(token), token)
                pif.add(codification['identifier'], uid)
            elif isConstant(token):
                uid = symbolTable.add(int(token.replace('\'', '')), token) if token.replace('\'', '').isdigit()\
